...
import { AbschnittsweiserKantenSeitenBezug, KantenSeitenbezug, Netzbezug } from 'src/app/shared/models/netzbezug';
...
describe(NetzbezugSelektionLayerComponent.name, () => {
  ...
  beforeEach(() => {
    ...
    netzbezugAuswahlModusService = mock<NetzbezugAuswahlModusService>();

    onMapClick$ = new Subject();
    when(olMapService.click$()).thenReturn(onMapClick$);

    return MockBuilder(NetzbezugSelektionLayerComponent, SharedModule)
      ...
      .provide({ provide: NetzbezugAuswahlModusService, useValue: instance(netzbezugAuswahlModusService) });
  });
  ...
  describe('constructor', () => {
    it('should disableLayers in viewer', () => {
      verify(netzbezugAuswahlModusService.startNetzbezugAuswahl()).once();
      expect().nothing();
    });
  });
  ...
  describe('onDestroy', () => {
    ...
    it('should enable layers', () => {
      verify(netzbezugAuswahlModusService.stopNetzbezugAuswahl()).never();
      component.ngOnDestroy();
      verify(netzbezugAuswahlModusService.stopNetzbezugAuswahl()).once();
      expect().nothing();
    });
  });
  ...
  describe('onDeselectSegment', () => {
    ...
    it('should update selection one-sided if selected segment is one-sided', () => {
      ...
      component.onDeselectSegment(2, defaultNetzbezug.kantenBezug[0].kanteId, KantenSeite.LINKS);
      expect(changeNetzbezugSpy).toHaveBeenCalled();
  ...
  describe('onChangeSegmentierung', () => {
    ...
    it('should update netzbezug on one side if change on one side', () => {
      ...
      component.onSegmentierungChanged([0, 0.2, 1], defaultNetzbezug.kantenBezug[0].kanteId, KantenSeite.LINKS);
      expect(changeNetzbezugSpy).toHaveBeenCalled();
      expect(changeNetzbezugSpy.calls.mostRecent().args[0]).toEqual({
        ...defaultNetzbezug,
        kantenBezug: [
