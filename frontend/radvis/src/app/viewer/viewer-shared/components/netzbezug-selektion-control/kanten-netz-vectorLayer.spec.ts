...
describe('KantenNetzVectorlayer', () => {
  ...
  describe('zweiseitigeNetzanzeige', () => {
    ...
    it('should set zweiseitigkeit correctly on Features', () => {
      ...
      expect(
        features.map(f => f.get(FeatureProperties.SEITE_PROPERTY_NAME)).filter(n => n === Seitenbezug.LINKS).length
      ).toEqual(2);
      expect(
        features.map(f => f.get(FeatureProperties.SEITE_PROPERTY_NAME)).filter(n => n === Seitenbezug.RECHTS).length
      ).toEqual(2);
