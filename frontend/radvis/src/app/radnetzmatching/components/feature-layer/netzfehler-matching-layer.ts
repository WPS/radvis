...
export class NetzfehlerMatchingLayer extends RadVisLayer {
  public static LAYER_ID = NetzfehlerLayer.ID_PREFIX + 'MATCHING';
  public static MIN_ZOOM_LEVEL = 11.2;

  constructor() {
    const color: Color = [255, 100, 0, 1];
    const lineColor: Color = [255, 100, 0, 0.5];
    const highlightColor: Color = [255, 100, 0, 1];

    super(
      NetzfehlerMatchingLayer.LAYER_ID,
      'Zuordnung fehlgeschlagen',
      `${NetzausschnittService.BASE_URL}/netzfehler/MATCHING`,
      ...
      color,
      LayerTypes.QUELLE,
      NetzfehlerMatchingLayer.MIN_ZOOM_LEVEL
    );
  }
}
