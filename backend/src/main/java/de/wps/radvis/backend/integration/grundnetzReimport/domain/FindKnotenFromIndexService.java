...
public class FindKnotenFromIndexService {
	...
	public KnotenTupel getNeuenKnotenTupelFuerUpdate(TopologischesUpdate topologischesUpdate, KnotenIndex knotenIndex)
		throws StartUndEndpunktGleichException {
		Point neuVonPoint = topologischesUpdate.getNeuVon();
		Point neuNachPoint = topologischesUpdate.getNeuNach();
		Knoten altVonKnoten = topologischesUpdate.getKante().getVonKnoten();
		Knoten altNachKnoten = topologischesUpdate.getKante().getNachKnoten();

		if (neuVonPoint.distance(altVonKnoten.getPoint()) > KnotenIndex.SNAPPING_DISTANCE
			&& neuNachPoint.distance(altNachKnoten.getPoint()) > KnotenIndex.SNAPPING_DISTANCE) {
			return findOrCreateKnotenTupel(neuVonPoint, neuNachPoint, knotenIndex);
		} else if (neuVonPoint.distance(altVonKnoten.getPoint()) > KnotenIndex.SNAPPING_DISTANCE) {
			return findOrCreateKnotenTupelMitKnotenGefixt(neuVonPoint, altNachKnoten, knotenIndex, true);
		} else {
			return findOrCreateKnotenTupelMitKnotenGefixt(neuNachPoint, altVonKnoten, knotenIndex, false);
		}
	}
}
