...
export namespace MassnahmenImportAttribute {
  ...
  export const pflichtAttributeOptions: EnumOption[] = pflichtAttribute.map(
    (k: string): EnumOption => {
      switch (k) {
        ...
        case MassnahmenImportAttribute.ZUSTAENDIGER:
          return { name: k, displayText: 'Zuständiger' };
  ...
  export const optionaleAttributeOptions: EnumOption[] = optionaleAttribute.map(
    (k: string): EnumOption => {
      switch (k) {
