...
import { AbschnittSelektion } from 'src/app/viewer/viewer-shared/components/netzbezug-selektion-control/netzbezug-selektion-layer/abschnitt-selektion';
...
export class NetzbezugSelektion {
  ...
  kanteSchneiden(kanteId: number, coordinate: Coordinate, seitenbezug?: Seitenbezug): void {
    ...
    abschnittSelektion.insertSegment(lineareReferenz, seitenbezug);
  }

  deselectSegment(kanteId: number, index: number, seitenbezug?: Seitenbezug): void {
    invariant(this.isKanteSelected(kanteId));
    const abschnittSelektion = this.getSelektionForKante(kanteId);
    abschnittSelektion.deselectSegment(index, seitenbezug);
    if (
      abschnittSelektion.getSelectedIndices(Seitenbezug.LINKS).length === 0 &&
      abschnittSelektion.getSelectedIndices(Seitenbezug.RECHTS).length === 0
    ) {
      this.deselectKante(kanteId);
    }
  }

  selectSegment(kanteId: number, index: number, seitenbezug?: Seitenbezug): void {
    invariant(this.isKanteSelected(kanteId));
    const abschnittSelektion = this.getSelektionForKante(kanteId);
    abschnittSelektion.selectSegment(index, seitenbezug);
  }

  selectKante(kanteId: number, geometrie: LineStringGeojson, seitenbezug?: Seitenbezug): void {
    invariant(!this.isKanteSelected(kanteId));
    this._kantenSeitenAbschnitte.push(
      new AbschnittSelektion(
        kanteId,
        geometrie,
        [{ von: 0, bis: 1, selected: seitenbezug === Seitenbezug.LINKS || seitenbezug === undefined }],
        [{ von: 0, bis: 1, selected: seitenbezug === Seitenbezug.RECHTS || seitenbezug === undefined }]
      )
    );
  }
  ...
  updateSegmentierung(kanteId: number, newSegmentierung: number[], seitenbezug?: Seitenbezug): void {
    invariant(this.isKanteSelected);
    this.getSelektionForKante(kanteId).updateSegmentierung(newSegmentierung, seitenbezug);
  }
  ...
  private parseKantenSelektion(netzbezug: Netzbezug | undefined): void {
    ...
    Netzbezug.groupByKante(netzbezug.kantenBezug).forEach((kantenBezuege, kanteId) => {
      const kantenBezuegeLinks = kantenBezuege.filter(
        kBZ => kBZ.seitenbezug === KantenSeitenbezug.BEIDSEITIG || kBZ.seitenbezug === KantenSeitenbezug.LINKS
      );
      const kantenBezuegeRechts = kantenBezuege.filter(
        kBZ => kBZ.seitenbezug === KantenSeitenbezug.BEIDSEITIG || kBZ.seitenbezug === KantenSeitenbezug.RECHTS
      );
