...
import { AbstellanlageDokumentListeResolverService } from 'src/app/viewer/abstellanlage/services/servicestation-dokument-liste-resolver.service';
...
describe(AbstellanlageDokumentListeResolverService.name, () => {
  ...
  it('should emit data if list remains empty', fakeAsync(() => {
    ...
    when(abstellanlageService.resolve(anything(), anything())).thenResolve(defaultAbstellanlage);
