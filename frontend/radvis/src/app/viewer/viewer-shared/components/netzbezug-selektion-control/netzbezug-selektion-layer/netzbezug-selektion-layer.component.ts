...
import { lineareReferenzierungLayerZIndex } from 'src/app/viewer/viewer-shared/models/viewer-layer-zindex-config';
...
export class NetzbezugSelektionLayerComponent implements OnDestroy, OnInit, OnChanges {
  ...
  RECHTS = Seitenbezug.RECHTS;
  LINKS = Seitenbezug.LINKS;
  minZoom = 16;
  ...
  onSegmentierungChanged(newSegmentierung: number[], kanteId: number, seitenbezug?: Seitenbezug): void {
    this.selectedNetzbezug.updateSegmentierung(kanteId, newSegmentierung, seitenbezug);
    this.netzbezugChange.emit(this.selectedNetzbezug.toNetzbezug());
  }

  onSelectSegment(event: SelectElementEvent, kanteId: number, seitenbezug?: Seitenbezug): void {
    ...
    if (this.pointSelectionMode) {
      ...
      this.selectedNetzbezug.kanteSchneiden(kanteId, event.clickedCoordinate, seitenbezug);
    } else {
      this.selectedNetzbezug.selectSegment(kanteId, event.index, seitenbezug);
    }

    this.netzbezugChange.emit(this.selectedNetzbezug.toNetzbezug());
  }

  onDeselectSegment(index: number, kanteId: number, seitenbezug?: Seitenbezug): void {
    if (this.pointSelectionMode) {
      return;
    }

    this.selectedNetzbezug.deselectSegment(kanteId, index, seitenbezug);
