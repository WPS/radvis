...
@Tag("group5")
@ContextConfiguration(classes = { LandesradfernwegeVariantenTfisImportJobTestIT.TestConfiguration.class,
	CommonConfiguration.class,
	GeoConverterConfiguration.class, })
@EnableConfigurationProperties(value = { CommonConfigurationProperties.class, FeatureToggleProperties.class,
	PostgisConfigurationProperties.class })
public class LandesradfernwegeVariantenTfisImportJobTestIT extends DBIntegrationTestIT {
	...
	LandesradfernwegeVariantenTfisImportJob job;
	...
	void setup() throws IOException, KeinMatchGefundenException {
		...
		job = new LandesradfernwegeVariantenTfisImportJob(jobExecutionDescriptionRepository, fahrradrouteRepository,
	...
	class With1FahrradrouteAnd2Varianten {
		...
		@Test
		void run_update1Insert1_onlyNetzbezug() {
			// arrange
			...
			Fahrradroute updatedFahrradroute = fahrradrouteRepository.findById(fahrradroute.getId()).get();
			assertThat(updatedFahrradroute.getVarianten()).hasSize(2);
			FahrradrouteVariante expectedVariante1 = FahrradrouteVariante.tfisVarianteBuilder().kategorie(kategorie2)
				.geometrie(geometrie1)
				...
				.linearReferenzierteProfilEigenschaften(List.of())
				.build();
			assertThat(updatedFahrradroute.findFahrradrouteVariante(expectedVariante1.getTfisId()).get())
				.usingRecursiveComparison()
				.usingOverriddenEquals()
				.isEqualTo(expectedVariante1);
			FahrradrouteVariante expectedVariante2 = FahrradrouteVariante.tfisVarianteBuilder().kategorie(kategorie2)
				...
				.linearReferenzierteProfilEigenschaften(List.of())
				.build();
			assertThat(updatedFahrradroute.findFahrradrouteVariante(expectedVariante2.getTfisId()).get())
				.usingRecursiveComparison()
				.ignoringOverriddenEqualsForTypes(FahrradrouteVariante.class)
				.ignoringFields("id")
				.usingOverriddenEquals()
		...
		@Test
		void run_insert1_noUpdateIfNetzbezug() {
			// arrange
			FahrradrouteVariante existingVariante1 = FahrradrouteVariante.tfisVarianteBuilder()
				.geometrie(geometrie2)
				.abschnittsweiserKantenBezug(
			...
			FahrradrouteVariante expectedVariante2 = FahrradrouteVariante.tfisVarianteBuilder().kategorie(kategorie2)
				...
				.linearReferenzierteProfilEigenschaften(List.of())
				.build();
			assertThat(updatedFahrradroute.findFahrradrouteVariante(varianteId2).get())
				.usingRecursiveComparison()
				.ignoringOverriddenEqualsForTypes(FahrradrouteVariante.class)
				.ignoringFields("id")
				.usingOverriddenEquals()
		...
		@Test
		void run_update1Insert1_noDelete() {
			// arrange
			FahrradrouteVariante existingVariante1 = FahrradrouteVariante.tfisVarianteBuilder()
				.abschnittsweiserKantenBezug(
					List.of(new AbschnittsweiserKantenBezug(netzbezugKante2, LinearReferenzierterAbschnitt.of(0, 1))))
				.kategorie(kategorie2).linearReferenzierteProfilEigenschaften(List.of())
				.tfisId(varianteId1).build();
			TfisId varianteId3 = TfisId.of("varianteId3");
			FahrradrouteVariante varianteNotInTfis = FahrradrouteVariante.tfisVarianteBuilder()
				.abschnittsweiserKantenBezug(
					List.of(new AbschnittsweiserKantenBezug(netzbezugKante1, LinearReferenzierterAbschnitt.of(0, 1))))
				.kategorie(kategorie1).linearReferenzierteProfilEigenschaften(List.of())
				.tfisId(varianteId3).build();
			Fahrradroute fahrradroute = fahrradrouteRepository
				.save(FahrradrouteTestDataProvider.onKante(netzbezugKante1)
					.verantwortlich(null)
					.tfisId(fahrradrouteId)
					.fahrradrouteTyp(FahrradrouteTyp.RADVIS_ROUTE)
					.kategorie(Kategorie.LANDESRADFERNWEG)
					.varianten(List.of(existingVariante1, varianteNotInTfis))
					.build());
			...
			// assert
			Fahrradroute updatedFahrradroute = fahrradrouteRepository.findById(fahrradroute.getId()).get();
			assertThat(updatedFahrradroute.getVarianten()).hasSize(3);
			FahrradrouteVariante expectedVariante1 = FahrradrouteVariante.tfisVarianteBuilder()
				.kategorie(existingVariante1.getKategorie())
				.geometrie(geometrie1)
				.id(fahrradroute.findFahrradrouteVariante(varianteId1).get().getId())
				.abschnittsweiserKantenBezug(
					List.of(new AbschnittsweiserKantenBezug(netzbezugKante1, LinearReferenzierterAbschnitt.of(0, 1))))
				.linearReferenzierteProfilEigenschaften(List.of())
				.tfisId(varianteId1)
			...
			assertThat(updatedFahrradroute.findFahrradrouteVariante(varianteId1).get()).usingRecursiveComparison()
				.usingOverriddenEquals()
				.isEqualTo(expectedVariante1);
			assertThat(updatedFahrradroute.findFahrradrouteVariante(varianteId3).get()).usingRecursiveComparison()
				.usingOverriddenEquals()
				.isEqualTo(varianteNotInTfis);
			FahrradrouteVariante expectedVariante2 = FahrradrouteVariante.tfisVarianteBuilder().kategorie(kategorie2)
				.geometrie(geometrie2)
				.tfisId(varianteId2)
				.abschnittsweiserKantenBezug(
	...
	class With2FahrradroutenAndJe1Variante {
		...
		@Test
		void with3fahrradrouten_deleteNoVariantenForFahrradroute3() {
			// arrange
			FahrradrouteVariante existingVariante1 = FahrradrouteVariante.tfisVarianteBuilder()
				.abschnittsweiserKantenBezug(
					List.of(new AbschnittsweiserKantenBezug(netzbezugKante2, LinearReferenzierterAbschnitt.of(0, 1))))
			...
			assertThat(updatedFahrradroute3.getVarianten()).hasSize(1);
