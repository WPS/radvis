...
export class ImportAttributeLayerComponent implements OnInit, OnDestroy {
  ...
  constructor(private olMapService: OlMapService, private manuellerImportService: ManuellerImportService) {
  ...
  private onModifyEnd = (event: ModifyEvent): void => {
    ...
    const request$ = this.manuellerImportService.updateMappedGrundnetzkanten({
      updatedLinestring: {
