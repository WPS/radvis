...
@Component({
  selector: 'rad-import-attribute-tool',
...
export class ImportAttributeToolComponent {
  constructor(attributeImportService: AttributeImportService, attributeImportRoutingService: AttributeRoutingService) {
    attributeImportService.getImportSession().subscribe(session => {
      if (!session) {
        attributeImportRoutingService.navigateToFirst();
        return;
      }

      attributeImportRoutingService.navigateToStep(session.schritt);
    });
  }
}
