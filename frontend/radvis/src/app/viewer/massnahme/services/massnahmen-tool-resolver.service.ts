...
export class MassnahmenToolResolverService {
  constructor(private massnahmeService: MassnahmeService) {}

  resolve(route: ActivatedRouteSnapshot): Promise<MassnahmeToolView> {
    const id = route.paramMap.get('id');
    if (id) {
      return this.massnahmeService.getMassnahmeToolView(+id);
    }
