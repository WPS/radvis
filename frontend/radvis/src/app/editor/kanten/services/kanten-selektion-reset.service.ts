...
export class KantenSelektionResetService implements CanDeactivate<AbstractAttributGruppeEditor> {
  constructor(
    private kantenSelektionService: KantenSelektionService,
    private netzBearbeitungModusService: NetzBearbeitungModusService
  ) {}
  ...
  canDeactivate(
    ...
    const noReset = nextState && this.netzBearbeitungModusService.isKantenSubEditorAktiv(nextState.url);
    return this.kantenSelektionService.cleanUp(!noReset);
  }
}
