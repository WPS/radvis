...
class DLMInitialImportJobTestIT extends DBIntegrationTestIT {
	...
	@Mock
	private DLMWFSImportRepository dlmWFSImportRepository;
	...
	void setup() {
		...
		dlmInitialImportJob = new DLMInitialImportJob(
			...
			entityManager,
			new de.wps.radvis.backend.integration.grundnetzReimport.domain.InitialPartitionenImportService(
	...
	public void sollKantenUndKnotenImportieren() {
		...
		when(dlmWFSImportRepository.readStrassenFeatures(bereich1)).thenReturn(
			Stream.of(importedFeature1, importedFeature2));
		when(dlmWFSImportRepository.readStrassenFeatures(bereich2)).thenReturn(
			Stream.of(importedFeature2, importedFeature3));
	...
	public void fehlschlag_KantenUndKnotenAufgeraeumt() {
		...
		when(dlmWFSImportRepository.getPartitionen()).thenReturn(partitions);
		when(dlmWFSImportRepository.readStrassenFeatures(bereich1)).thenReturn(
			Stream.of(ImportedFeatureTestDataProvider.defaultWFSObject().build()));
		when(dlmWFSImportRepository.readStrassenFeatures(bereich2)).thenThrow(new RuntimeException());
