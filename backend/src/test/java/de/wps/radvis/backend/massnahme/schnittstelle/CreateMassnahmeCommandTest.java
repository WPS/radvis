...
public class CreateMassnahmeCommandTest {
	Validator javaxValidator = Validation.buildDefaultValidatorFactory().getValidator();

	@Test
	void validate_default() {
		assertThat(javaxValidator.validate(CreateMassnahmeCommandTestDataProvider.defaultValue().build())).isEmpty();
	}

	@Test
	void validate_begruendungZurueckstellung() {
		assertThat(javaxValidator.validate(CreateMassnahmeCommandTestDataProvider.defaultValue()
		...
		assertThat(javaxValidator.validate(CreateMassnahmeCommandTestDataProvider.defaultValue()
			.umsetzungsstatus(Umsetzungsstatus.PLANUNG).zurueckstellungsGrund(null)
			.begruendungZurueckstellung(BegruendungZurueckstellung.of("Test")).build()))
				.isNotEmpty();
		assertThat(javaxValidator.validate(CreateMassnahmeCommandTestDataProvider.defaultValue()
			.umsetzungsstatus(Umsetzungsstatus.IDEE).zurueckstellungsGrund(null).begruendungZurueckstellung(null)
			.build())).isEmpty();
	}
}
