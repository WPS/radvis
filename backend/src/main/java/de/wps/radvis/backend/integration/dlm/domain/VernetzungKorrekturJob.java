...
public class VernetzungKorrekturJob extends AbstractJob {
	/*
	 * Der hier verwendete Wert basiert auf dem Prod-Datenstand vom 16.02.23 und ist so gewaehlt, dass bei allen zu
	 * diesem Zeitpunkt sinnvollen Stellen die Radvis Kante auf den naechsten Knoten snappt.
	 */
	...
	protected Optional<JobStatistik> doRun() {
		...
		vernetzungKorrekturJobStatistik.anzahlKnotenGeloescht += netzService
			.deleteVerwaisteDLMKnoten(NetzAenderungAusloeser.VERNETZUNG_KORREKTUR,
				vernetzungKorrekturJobStatistik.radvisKantenVernetzungStatistik.knotenDeleteStatistik);
	...
	protected void entferneFalscheVernetzung(VernetzungKorrekturJobStatistik statistik) {
		...
		knotenMitAuseinanderLiegendenKantenEnden.forEach(knoten -> {
			...
			Optional<Point> nearestLineEnding = adjazenteKanten
				.stream()
				.map(kante -> kante.getVonKnoten().equals(knoten) ? kante.getGeometry().getStartPoint()
					: kante
			...
			adjazenteKanten
				.forEach(kante -> {
					...
					Point lineEnding = isVonKnoten ? kante.getGeometry().getStartPoint()
						: kante.getGeometry()
							.getEndPoint();
					Point otherLineEnding = isVonKnoten ? kante.getGeometry().getEndPoint()
						: kante.getGeometry()
							.getStartPoint();
