...
import { KantenSeitenbezug, Netzbezug } from 'src/app/viewer/viewer-shared/models/netzbezug';
...
describe(NetzbezugHighlightLayerComponent.name, () => {
  ...
  beforeEach(() => {
    ...
    return MockBuilder(NetzbezugHighlightLayerComponent, ViewerModule)
      .provide({
  ...
  describe('ngOnChanges', () => {
    ...
    it('should respect linear referenz und seitenbezug', () => {
      ...
      const featuresLinks = features.filter(f => f.get(FeatureProperties.SEITE_PROPERTY_NAME) === Seitenbezug.LINKS);
      ...
      const featuresRechts = features.filter(f => f.get(FeatureProperties.SEITE_PROPERTY_NAME) === Seitenbezug.RECHTS);
