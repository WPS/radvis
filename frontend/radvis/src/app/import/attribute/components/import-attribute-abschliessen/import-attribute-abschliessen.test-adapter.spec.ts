...
export class ImportAttributeAbschliessenTestAdapter {
  ...
  doesUebernehmenButtonExist(): boolean {
    return !!this.uebernehmenButton && this.uebernehmenButton.nativeElement.textContent?.trim() === 'Übernehmen';
  }

  isUebernehmenButtonDisabled(): boolean {
    return !!this.uebernehmenButton && this.uebernehmenButton.nativeElement.disabled;
  }

  doesBeendenButtonExist(): boolean {
    return !!this.beendenButton && this.beendenButton.nativeElement.textContent?.trim() === 'Import beenden';
  }

  isBeendenButtonDisabled(): boolean {
    return !!this.beendenButton && this.beendenButton.nativeElement.disabled;
  }
  ...
  private get beendenButton(): DebugElement {
    return this.buttons?.[0];
  }

  private get uebernehmenButton(): DebugElement {
    return this.buttons?.[2];
  }

  private get buttons(): DebugElement[] {
    return this.component.queryAll(By.css('.import-step-footer-buttons button'));
  }
}
