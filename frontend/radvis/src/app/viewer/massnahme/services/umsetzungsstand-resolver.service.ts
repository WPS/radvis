...
export class UmsetzungsstandResolverService {
  constructor(private massnahmeService: MassnahmeService) {}

  resolve(route: ActivatedRouteSnapshot): Promise<Umsetzungsstand> {
    const id = route.parent?.paramMap.get('id');
    if (id) {
      return this.massnahmeService.getUmsetzungstand(+id);
    }
