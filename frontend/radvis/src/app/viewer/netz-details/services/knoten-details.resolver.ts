...
export const knotenDetailsResolver: ResolveFn<KnotenDetailView> = (route, state) => {
  const netzService: NetzdetailService = inject(NetzdetailService);
  const id = route.paramMap.get('id');
  invariant(id, 'Knoten-ID muss als Parameter id an der Route gesetzt sein.');
  return netzService.getKnotenForView(+id);
};
