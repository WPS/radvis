...
public class VernetzungService {
	...
	/*
		Achtung:
		Diese Methode geht davon aus, dass keine verwaisten Dlm Knoten existieren.
		Die aufrufende Methode ist dafuer verantwortlich, dass dieses require erfuellt ist:
	...
	public void vernetzeAlleRadvisKantenNeu(RadvisKantenVernetzungStatistik radvisKantenVernetzungStatistik,
		...
		kantenRepository.findKanteByQuelle(QuellSystem.RadVis).forEach(radvisKante -> {
			log.info("Vernetzung für Radvis-Kante [ID=" + radvisKante.getId() + "] wird überprüft...");
	...
	private Optional<Knoten> findNewKnoten(Point atPosition, Knoten alterKnoten,
		...
		if (!newNetzknoten.isPresent()) {
			...
			log.info(
				"Von-Knoten " + alterKnoten
					+ " wird noch von RadVIS-Kante referenziert und behalten");
		}
