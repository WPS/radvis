...
export class DateiLayerVerwaltungComponent implements OnInit {
  ...
  onManageStyles(layer: DateiLayer): void {
    ...
    this.dialog.open(this.styleDialogRef, {
      id: this.STYLE_DIALOG_ID,
      data: { layer, formControl },
      autoFocus: 'dialog',
    });
  }
  ...
  public onDeleteLayer(id: number): void {
    this.dateiLayerService
      .delete(id)
      .then(() => {
        this.notifyUserService.inform('Kartenebene erfolgreich gelöscht.');
        this.refreshLayers();
      })
      .catch(reason => {
