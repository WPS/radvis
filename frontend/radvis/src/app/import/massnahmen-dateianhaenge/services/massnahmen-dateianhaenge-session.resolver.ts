...
export const massnahmenDateianhaengeSessionResolver: ResolveFn<MassnahmenDateianhaengeImportSessionView> = () => {
  const service: MassnahmenDateianhaengeService = inject(MassnahmenDateianhaengeService);
  return service
    .getImportSession()
    .pipe(filter((session): session is MassnahmenDateianhaengeImportSessionView => !!session));
};
