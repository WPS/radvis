...
@Component({
  selector: 'rad-leihstation-layer',
  template: '',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class LeihstationLayerComponent extends AbstractInfrastrukturLayerComponent<Leihstation> implements OnDestroy {
  private layer: VectorLayer;

  constructor(
    routingService: LeihstationRoutingService,
    filterService: LeihstationFilterService,
    featureHighlightService: FeatureHighlightService,
    private olMapService: OlMapService
  ) {
    super(routingService, filterService, featureHighlightService, LEIHSTATIONEN);
  ...
  protected convertToFeature(infrastruktur: Leihstation): Feature<Geometry>[] {
    ...
    feature.set(
      ...
      'Leihstation' + (infrastruktur.betreiber ? ` (${infrastruktur.betreiber})` : '')
    );
    return [feature];
  }
